...
Minimize a function: partial derivative 
\[ f(w) \mapsto \frac{\partial(f(w))}{\partial(w_i)} \quad where \quad f(w) = h_w(x^{(i)})\]

\begin{enumerate}
    \item Take the partial derivative of the linear model
    \[\frac{\partial(h_w(x^{(i)}))}{\partial w_j} = \frac{\partial(w^Tx^{(i)})}{\partial w_j} = \frac{\partial(\sum_{i=1}^{N}{w_ix_i})}{\partial w_j}\]
    \item Take the par. deriv. of each term in the sum, with respect to $w_0$ and $w_1$
    \[\frac{\partial((h_w(x^{(i)})-y)^2)}{\partial(w_j)} = 2 (h_w(x^{(i)}) - y) x_j\]
    Where 
    \[h_w(x^{(i)}) = w_0 + w_1(x^{(i)})\]
    \[\frac{\partial }{\partial w_0} \left[ \frac{1}{2N} \sum_{i=1}^{N} \left((w_0 + w_1 x^{(i)}) - y^{(i)}\right)^2 \right]= \frac{1}{N} \sum_{i=1}^{N}((w_0 + w_1 x^{(i)}) - y^{(i)}) \quad {(x_0 = 1)}\]
    \[\frac{\partial }{\partial w_1} \left[ \frac{1}{2N} \sum_{i=1}^{N} \left((w_0 + w_1 x^{(i)}) - y^{(i)}\right)^2 \right]= \frac{1}{N} \sum_{i=1}^{N}((w_0 + w_1 x^{(i)}) - y^{(i)})(x^{(i)})\]
...
\[\frac{\partial(J_{MSE})}{\partial(w_j)} = \frac{1}{N} \sum_{i = 1}^{N} (w^Tx^{(i)} - y^{(i)}) = 0\]
Where \(w = 
\begin{bmatrix}
    w_0 \\w_1 \\\dots \\w_N
